  ...
  (define value-of
    (lambda (exp env)
	  ...
	  (when (instrument-let)
        ...
        (proc-exp (var body)
	  (proc-val
	    (cons (procedure var body env) #f)))

        (vproc-exp (var body)
          (proc-val (cons (procedure var body env) #t)))

        (call-exp (rator rand)
          (let* ((pair (expval->proc (value-of rator env)))
                 (by-val (cdr pair))
                 (proc (car pair))
                 (arg (value-of-operand rand env by-val)))
