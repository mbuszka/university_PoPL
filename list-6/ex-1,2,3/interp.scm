...
;;;;;;;;;;;;;;;; the interpreter ;;;;;;;;;;;;;;;;
          ...
          (let ((val1 (value-of exp1 env)))
            (let ((num1 (expval->num val1)))
              (if (zero? num1)
                (bool-val #t)
                (bool-val #f)))))
        (not-exp (exp1) (bool-val (not (expval->bool (value-of exp1 env)))))
  ...
  (define (apply-smt smt env)
    (cases statement smt
      (assign-smt (var exp)
        (setref! (apply-env env var) (value-of exp env)))
      (print-smt (exp) (add-print (value-of exp env)))
      (block-smt (smt1 smts)
        (for-each (lambda (s) (apply-smt s env)) (cons smt1 smts)))
      ...
      (while-smt (exp smt1)
        (let loop ()
          (when (expval->bool (value-of exp env))
            (apply-smt smt1 env)
            (loop))))
